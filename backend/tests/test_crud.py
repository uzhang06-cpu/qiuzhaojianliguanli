...
class TestCreatePosition:
    ...
    def test_create_basic(self, client: TestClient):
        resp = client.post("/positions", json=_make_position_data())
    ...
    def test_create_auto_logs_status(self, client: TestClient):
        ...
        log_resp = client.get(f"/status-logs?position_id={pos_id}")
...
class TestListPositions:
    ...
    def test_filter_by_status(self, client: TestClient):
        self._create_seed_data(client)
        resp = client.get("/positions?status=applied")
        data = resp.json()
        assert len(data) == 1
    ...
    def test_search_by_keyword(self, client: TestClient):
        self._create_seed_data(client)
        resp = client.get("/positions?keyword=字节")
        data = resp.json()
        assert len(data) == 1
    ...
    def test_sort_by_company_asc(self, client: TestClient):
        self._create_seed_data(client)
        resp = client.get("/positions?sort_by=company&sort_dir=asc")
        data = resp.json()
...
class TestGetPosition:
    ...
    def test_get_by_id(self, client: TestClient):
        create_resp = client.post("/positions", json=_make_position_data())
        pos_id = create_resp.json()["id"]

        resp = client.get(f"/positions/{pos_id}")
        assert resp.status_code == 200
        assert resp.json()["company"] == "字节跳动"

    def test_get_not_found(self, client: TestClient):
        resp = client.get("/positions/99999")
...
class TestStatusTransition:
    ...
    def _create_position(self, client: TestClient, status: str = "interested"):
        resp = client.post("/positions", json=_make_position_data(status=status))
        return resp.json()["id"]

    def test_forward_transition(self, client: TestClient):
        pos_id = self._create_position(client)
        resp = client.post(
            f"/positions/{pos_id}/status",
...
class TestDeletePosition:
    ...
    def test_soft_delete(self, client: TestClient):
        ...
        resp = client.delete(f"/positions/{pos_id}")
        assert resp.status_code == 200

        # 活跃列表中不应出现
        list_resp = client.get("/positions?is_active=true")
        assert all(p["is_active"] for p in list_resp.json())

    def test_delete_not_found(self, client: TestClient):
        resp = client.delete("/positions/99999")
        assert resp.status_code == 404
